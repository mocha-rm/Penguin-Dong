...
public static class Util
{
    ...
    public static Transform FindChildWithPath(string path)
    {
        ...
        GameObject rootObject = GameObject.Find(pathElements[0]);
        if (rootObject == null)
        {
            Debug.LogError($"{pathElements[0]} : Root Can't Find");
            return null;
        }
        ...
        {
            transform = transform.Find(pathElements[i]);
            if (transform == null)
            {
                return null;
            }
    ...
    public static Color PingPongColor(Color color, Color toColor, float factor, float length = 1f)
    {
        var t = Mathf.PingPong(factor, length);
